...
def crypto_check():
    ...
    if int(vers_split[1]) > 7 and Crypto.__version__ == "2.6.1":
        logging.error(
            "PyCrypto is broken on Python 3.8+. Please replace it with pycryptodome before running again. Exiting in 10 sec..."
        )
...
async def start_webserver():
    ...
    logging.info(
        "Starting webserver at {listen}:{port}".format(listen=listen, port=port)
    )
